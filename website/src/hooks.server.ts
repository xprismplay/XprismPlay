...
async function initializeScheduler() {
  ...
  try {
    ...
    if (result === "OK") {
      ...
      function getMillisUntilNextPayoutWindow(): number {
        const now = new Date();
        const next = new Date(
          Date.UTC(
            now.getUTCFullYear(),
      ...
      async function scheduleDividendPayout() {
        const delay = getMillisUntilNextPayoutWindow();
        setTimeout(async () => {
          try {
            const results = await processAllDividendPayouts();
            if (results.length > 0) {
              console.log(
                `💰 Dividend payouts: ${results.length} coins paid out`,
              );
            }
          } catch (err) {
            console.error("Dividend payout error:", err);
          }
    ...
    if (cached && now - cached.timestamp < cached.ttl) {
      ...
      if (userRecord?.isBanned) {
        ...
        if (event.url.pathname !== "/banned") {
          const banReason = encodeURIComponent(
        ...
        const [spendResult] = await db
          .select({
            total: sql<number>`COALESCE(SUM(${gemTransactions.usdAmount}), 0)`,
          })
        ...
        userData = {
          ...
          isAdmin:
            hasFlag(userRecord.flags, "IS_HEAD_ADMIN", "IS_ADMIN") || false,
          image: userRecord.image || "",
          isBanned: userRecord.isBanned || false,
          banReason: userRecord.banReason,
          avatarUrl: userRecord.image,
          baseCurrencyBalance: parseFloat(
            userRecord.baseCurrencyBalance || "0",
          ),
          bio: userRecord.bio || "",
