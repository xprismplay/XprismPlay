...
async function advanceOrSettle(session: BlackjackSession, userId: number) {
	...
	if (nextIndex !== -1) {
		...
		await redis.set(getSessionKey(session.sessionToken), JSON.stringify(session), {
			EX: sessionTtl
		});
	...
	try {
		...
		if (action === 'deal') {
			...
			const result = await db.transaction(async (tx) => {
				...
				const currentBalance =
					Math.round(Number(userData.baseCurrencyBalance) * 100000000) / 100000000;

				if (roundedBet > currentBalance) {
					throw new Error(
						`Insufficient funds. You need $${roundedBet.toFixed(2)} but only have $${currentBalance.toFixed(2)}`
					);
				}
			...
			if (result.settled) {
				const won = result.session.playerHands.some(
		...
		if (action === 'insurance') {
			...
			if (takingInsurance) {
				...
				if (halfBet > currentBalance) {
					throw new Error(
						`Insufficient funds. You need $${halfBet.toFixed(2)} but only have $${currentBalance.toFixed(2)}`
					);
				}
		...
		if (action === 'hit') {
			...
			if (isBust(currentHand.cards)) {
				...
				const {
					...
					newBalance,
					settled
				} = await advanceOrSettle(session, userId);

				if (settled) {
					const won = updatedSession.playerHands.some(
						(h) => h.status === 'won' || h.status === 'blackjack'
					);
		...
		if (action === 'double') {
			...
			if (extraBet > currentBalance) {
				throw new Error(
					`Insufficient funds. You need $${extraBet.toFixed(2)} but only have $${currentBalance.toFixed(2)}`
				);
			}
		...
		if (action === 'split') {
			...
			if (splitBet > currentBalance) {
				throw new Error(
					`Insufficient funds. You need $${splitBet.toFixed(2)} but only have $${currentBalance.toFixed(2)}`
				);
			}
