...
export const GET: RequestHandler = async ({ request }) => {
	...
	const conditions = [
		and(eq(chatChannel.type, 'DIRECT'), or(eq(chatChannel.user1Id, userId), eq(chatChannel.user2Id, userId)))
	];
	...
	const channels = await db.select().from(chatChannel).where(or(...conditions));
	...
	if (userIdsToFetch.size > 0) {
		const fetchedUsers = await db.select({
			...
			image: user.image
		}).from(user).where(inArray(user.id, Array.from(userIdsToFetch)));
		
		fetchedUsers.forEach(u => usersInfo.set(u.id, u));
	}

	// Fetch or create Global Public Chat
	let [globalChat] = await db.select().from(chatChannel).where(
		and(eq(chatChannel.type, 'DIRECT'), eq(chatChannel.user1Id, null), eq(chatChannel.user2Id, null))
	).limit(1);

	if (!globalChat) {
		const newGlobal = await db.insert(chatChannel).values({
			type: 'DIRECT',
			user1Id: null,
			user2Id: null
		}).returning();
...
export const POST: RequestHandler = async ({ request }) => {
	...
	let channel = await db.select().from(chatChannel).where(
		and(
	...
	if (channel.length === 0) {
		const newChannel = await db.insert(chatChannel).values({
			type: 'DIRECT',
			user1Id,
			user2Id
		}).returning();
