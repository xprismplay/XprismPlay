...
export const GET: RequestHandler = async ({ request }) => {
	...
	// Get friends
	const friendships = await db.select().from(friendship).where(or(
		eq(friendship.user1Id, userId),
		eq(friendship.user2Id, userId)
	));
	...
	const pendingRequestsRaw = await db.select({
		...
		and(eq(friendRequest.receiverId, userId), eq(friendRequest.status, 'PENDING'))
	);
	...
	if (pendingRequestsRaw.length > 0) {
		...
		const senders = await db.select({
			...
			or(...senderIds.map(id => eq(user.id, id)))
		);
	...
	const sentRequestsRaw = await db.select({
		id: friendRequest.id,
		receiverId: friendRequest.receiverId,
		createdAt: friendRequest.createdAt
	}).from(friendRequest).where(
		and(eq(friendRequest.senderId, userId), eq(friendRequest.status, 'PENDING'))
	);
	...
	if (sentRequestsRaw.length > 0) {
		...
		const receivers = await db.select({
			...
			or(...receiverIds.map(id => eq(user.id, id)))
		);
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (action === 'SEND') {
		...
		const existingFriendship = await db.select().from(friendship).where(
			or(
		...
		const existingRequest = await db.select().from(friendRequest).where(
			or(
		...
		const req = await db.select().from(friendRequest).where(
			and(
		...
		await db.delete(friendRequest).where(
			and(
				eq(friendRequest.senderId, targetId), 
				eq(friendRequest.receiverId, userId)
			)
		...
		await db.delete(friendship).where(
			and(eq(friendship.user1Id, user1Id), eq(friendship.user2Id, user2Id))
		);
		return json({ success: true });
	}
