...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!username || typeof level !== 'number' || isNaN(level) || level < 0) {
		return json({ message: 'Invalid input. Provide a username and a valid prestige level (>= 0).' }, { status: 400 });
	}
